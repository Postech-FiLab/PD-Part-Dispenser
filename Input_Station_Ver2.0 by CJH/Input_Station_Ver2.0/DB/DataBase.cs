    ...
    public class DataBase
    {
        ...
        "192.168.50.2", "3306", "postech_tb", "root", "filab1020");
            try
            {
                ...
                dbConnection.Open();
                Console.WriteLine(" * Postech_tb DB 오픈 완료");
            }
            catch (Exception e)
            {
        ...
        public string[] Select_all()
        {
            ...
            Console.WriteLine("");
            Console.WriteLine("-------------Part Dispenser Table------------");
        ...
        public Double get_deck_position(int deck_id)
        // deck_id에 맞는 decl_position 가져오기
        {
            string[] row1 = new string[3];
            DataTable dataTable = new DataTable();
            string select_all_sql = "SELECT * FROM postech_tb.part_dispenser" +
            ...
            DataRow dr = dataTable.Rows[0];
            Console.WriteLine("{0}, {1}, {2}", dr[0], dr[1], dr[2]);
            Console.WriteLine(dr[2].GetType());
